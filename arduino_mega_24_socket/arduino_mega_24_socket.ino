...
char* fetchCV (){
  ...
  for( int i = 0; i < 8; i++){
    cv_array[i] = measureCurrent(current_sensors[i]);
  }
 for(int channel = 0; channel < 16; channel++){
   // select channel 
    ...
    cv_array[i+8] = ftostr(measureCurrent(SIG));
  }
  cv_array[24] = ftostr(measureVoltage(voltage_sensor));


  int formatted_cv_state_string_buffer = 0;
  for (int i = 0; i < 25; i++) {
    formatted_cv_string_buffer += snprintf(NULL, 0, "%s", cv_array[i]);
  }

  // Add space for separators (e.g., commas) and a null terminator
  formatted_cv_state_string_buffer += 25 - 1;
  ...
  for (int i = 0; i < arraySize; ++i) {
    int charsWritten = snprintf(formattedString + offset, bufferSize - offset, "%s", myStrings[i]);
    offset += charsWritten;
    if (i < arraySize - 1) {
      // Add a separator (e.g., comma) between elements
      snprintf(formattedString + offset, bufferSize - offset, ",");
      ++offset;
    }
  }
